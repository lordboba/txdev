...
- **Tassel:** 28 strands merged into a vertex-coloured cone (`#bf242a` → `#8f1c22`), knot sphere `#9d2933`, a second pendulum hung from the bottom collar: it restores toward **plumb** (relative angle → −θ), is driven by the collar's tangential acceleration `(1 + R/l)·θ''` (R pivot → collar, l the tassel, 0.32 body widths), the drive capped at **6** (`PENDULUM.tassel.maxDrive`: on lantern B the 176 px cord over an 18 px tassel gave 13.4 and a cursor push pinned the strands on the clamp for 80 ms, a hard stop), ω = 2π/(0.45·T), ζ 0.35 (the tassel-to-cord length ratio √(24/118) ≈ 0.45, so 1.26 / 1.08 / 0.95 s), a cubic restoring term `−ω²·(ψ/0.5)³` from `softRad` outward (the strands' weight leaning against a large relative angle, so the limit is a weight, not a wall), relative angle clamped ±0.35 rad as a safety only. At the first-gust onset the strands trail the collar (relative ≈ −1× θ), then whip through plumb and peak past the body 50–150 ms after the body's peak. A 600 px sweep at 1200 px/s leaves every relative angle ≤ 12° (sim test; A 9.8°, B 12.1°). On a route change the new lower-in re-hangs the lantern fresh (θ, ψ, bob and their rates all 0): the pivot moved, so the old page's swing has no meaning on the new one. (A spring toward the body axis gave ≈ 16% relative amplitude, 0.4 px at the tip: welded to the lantern.)
...
| `touchstart` (mobile)     | tap position                                                                              | local gust A 0.6, `(1 - cos)` over 1.2 s, radius 0.35 vw, **plus** a one-off kick `θ̇ += 0.2 rad/s × falloff` on every hung lantern in the radius (`sim.tap`; the gust alone moved the 44 px lantern half a pixel: a tap on the H1 now swings it ≥ 2°, a tap on the lantern ≈ 4°) | `× (1 - smoothstep(0, 0.35·vw, dist))` for the kick too                                                        | at most once per 3 s |
...
| Tassel               | ω 2π/(0.45·T), ζ 0.35 (1.26 / 1.08 / 0.95 s), restoring toward plumb, driven `(1 + R/l)·θ''` capped at 6, cubic soft limit from 0.5 rad, relative angle ±0.35 rad as a safety; trails the collar at a gust's onset, whips past the body 50–150 ms after its peak (M5 measures the hero); ≤ 12° relative under a 1200 px/s sweep                                                                                                                                                                                                                                                                                            |
...
| Depth bands          | 3: scale 1.0 / 0.7 / 0.45, alpha 1.0 / 0.7 / 0.45, pointer parallax 1.0 / 0.6 / 0.3 of a (8, 4) px offset at the pointer's full travel (normalised pointer −1..1, damped λ 6, decaying to 0 when the pointer leaves the window; none in reduced motion); the far band holds leaves and ginkgo only (10–18 px after the 0.45 scale); florets keep to bands 0–1, because a 6–10 px corolla scaled 0.45 is a 2–3 px soft dot, and §5.4 refuses dust                                                                                                                                                                           |
...
**A3. The riddle slip (灯谜)** on lantern B. At rest: a vertical paper strip 38 × ≈ 150 px hung 14 px under the bottom collar, showing only the 谜目 in vertical upright writing (`打一项目` "guess one project" or `打一文` "guess one post"), Noto Serif SC 500 at 15 px, tracking 0.18 em, ink `#2b241c`, with a Plex Mono `--label-xs` index (`03/12`, tabular, no spaces: five glyphs at 10 px fit the 36 px strip interior) at the top and the 9 px 银朱 seal at the foot. Top edge lit by the lantern above: `linear-gradient(180deg, rgba(255,182,30,.22), transparent 38%)`, 1 px `#d9cbb0` edge. The strip is a `<button aria-expanded>` with a 44 px padded hit area; on hover / focus / tap it "pulls" (translateY +6 px, 180 ms, enter), and a card unfolds 8 px under the strip's foot, inside the gutter (max-width 28ch of its own Cormorant italic = 236 px, narrowed to the gutter and kept 8 px clear of the viewport edge and 8 + 18 px clear of the copy column, the 18 being the foot's sideways travel at a 2.2 gust (152 px × sin(0.8 × 8.6°)), so the 8 px rule holds at the peak; `text-wrap: pretty`, `hanging-punctuation: first`; `layout.ts` resolves the rect against the same exclusions as the strip and a slip whose card cannot be placed is not hung) with the 谜面 in **Cormorant Garamond italic 500, 19 px, line-height 1.3** on `#efe4cf`; 600 ms later the 谜底 line: `谜底 · iCalarms ↗` in Cormorant Garamond 500, 17 px, `lining-nums`, underlined in `--accent`, an `<a>` to `project.link` (the arrow is an inline SVG: no served font has U+2197, and a system glyph broke the underline). The line is a wrapping flex row (label, then the link as one unbreakable unit): on the 160 px `/past-experience` card a long title drops to its own line whole, never 'Med / Negotiate'. The button toggles (a second Enter, Space or click folds the card, as `aria-expanded` promises); Escape closes (wherever focus is) and returns focus to the strip button (`aria-controls` points at the card); a hover-opened card closes 400 ms after the pointer leaves, and an invisible hover bridge (`.slip[data-open]::after`, from the strip's mid-height down to the card's top over the card's width, plus 8 px around the card) keeps the diagonal from the strip to the 谜底 link inside the wrapper, so the grace is a backstop, not the rule (on `/schedule-a-call` the diagonal crossed ≈ 50 px of bare page and a slow hand lost the card); a focus- or click-opened card stays until blur or Escape: a click on a hover-opened slip promotes it to click-opened and keeps it open; mobile has no slips. `aria-label="Lantern riddle: {clue}. Answer: {answer}"`. Positioned each frame from the lantern's bottom-collar world point (`damp` λ 12); the strip inherits 0.8× θ, and the card counter-rotates (`rotate(−1 × --slip-theta)`, origin its top centre) so it hangs level from the swinging foot and only translates with it. Contrast of ink on slip paper ≥ 7:1.
