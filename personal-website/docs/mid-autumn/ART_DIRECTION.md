...
- **Tassel:** 28 strands merged into a vertex-coloured cone (`#bf242a` → `#8f1c22`), knot sphere `#9d2933`, on its own spring (ω = 2π/(0.45·T), ζ 0.35: the tassel-to-cord length ratio √(24/118) ≈ 0.45, so 1.26 / 1.08 / 0.95 s → trails the first-gust peak by 100–150 ms with ≈ 16% relative amplitude; a fixed 0.55 s spring could not lag a 2.8 s body at all).
...
- **Pool:** instanced quad at lantern z − 0.2, `NormalBlending`, elliptical falloff `pow(1 - r_ellipse, 2.0)`, 3.2× body width, 1.35× taller than wide, centred 0.4 body-heights below the lantern, colour per the hue gate (`paperHot` on `/`), peak alpha 0.14 dark / 0.22 on the grey studio / 0 light.
...
| Tassel                                     | from arrival                                                                                                                                                                     | spring                      | ω 2π/(0.45·T), ζ 0.35                                      | First kick from the arrival overshoot                                                                                                                                                                                                                                                                                         |
...
| Tassel               | ω 2π/(0.45·T), ζ 0.35 (1.26 / 1.08 / 0.95 s)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- M5. A scripted pointer sweep at 1200 px/s deflects the nearest lantern ≥ 8° and the farthest ≤ 3°; the poem column leans ≤ 0.6°; no attraction. Tassel peak (relative angle, `state().lanterns[i].tasselTheta`) lags body peak 80–150 ms at the first-gust peak.
